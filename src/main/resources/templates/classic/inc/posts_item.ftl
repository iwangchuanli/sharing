        ...
        <#if row.thumbnail?? && row.thumbnail?length gt 0>
            <div class="content-box">
                <div class="posts-item-img">
                    <a href="${base}/post/${row.category}-${row.id}.html" title="${row.title}">
                ...
                <div class="posts-item posts-item-gallery">
                    <h2><a href="${base}/post/${row.category}-${row.id}.html"
                           title="${row.title}"><#if escape>${row.title?html}<#else>${row.title}</#if></a></h2>
                    <div class="item-text">${row.summary}</div>
                    <div class="item-info">
                        <ul>
                            <li class="post-author hidden-xs">
                                <div class="avatar">
                                    <img alt="avaterImg" src="<@resource src=row.author.avatar + '?t=' + .now?time/>"
                                         class="lazy avatar avatar-50 photo" height="50" width="50">
                                </div>
                                <a title="${row.author.name}" href="${base}/users/${row.author.id}"
                                   target="_blank">${row.author.name}</a>
                            </li>
                            ...
                            <li class="ico-like hidden-xs"><i class="icon-star"></i>${row.category}</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="content-box posts-aside">
                <div class="posts-item">
                    <div class="item-title">
                        <h2><a title="${row.title}"
                               href="${base}/post/${row.category}-${row.id}.html"><#if escape>${row.title?html}<#else>${row.title}</#if></a>
                        </h2>
                    </div>
                    <div class="item-text">${row.summary}</div>
                    <div class="item-info">
