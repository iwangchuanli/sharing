	...
	<div class="panel-body">
		...
		<ul class="list">
            ...
            <#--<li>${row_index + 1}. <a href="${base}/post/${row.id}.html">${row.title}</a></li>-->
				<li><i class="fa fa-flag-o"></i>&nbsp; <a class="hosttest_post_a" href="${base}/post/${row.category}-${row.id}.html" title="${row.title}">${row.title}</a></li>
			</#list>
		</ul>
		</@sidebar>
	</div>
...
<div class="panel panel-default widget">
	...
	<div class="panel-body">
		<@sidebar method="latest_posts">
			<ul class="list">
				<#list results as row>
					<li><i class="fa fa-paper-plane-o"></i>&nbsp;  <a class="new_public_a" href="${base}/post/${row.category}-${row.id}.html" title="${row.title}">${row.title}</a></li>
				</#list>
			</ul>
		</@sidebar>
	</div>
...
<div class="panel panel-default widget">
    ...
    <div class="panel-body">
		<@sidebar method="latest_comments">
			<ul class="list">
				<#list results as row>
					<li><i class="fa fa-commenting-o"></i>&nbsp;  <a class="new_comment_a" href="${base}/post/${row.category}-${row.postId}.html" title="${row.content}">${row.content}</a></li>
				</#list>
			</ul>
		</@sidebar>
    </div>
